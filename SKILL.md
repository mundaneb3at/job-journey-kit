---
name: job-journey
description: Run a job / internship / co-op search as three ledgers — AMMUNITION (verified facts about you), SEARCH (find postings and prove they are live), FINDINGS (one tracker with an activity log) — plus four prompts that take one live posting through evaluation, tailoring, cover letter and interview prep, and four lane protocols (SEARCH, RESUME, LETTERS, OUTREACH) with gates you can run as optional Python tools. Use when someone says "help me find a job", "start my job search", "is my resume claim true", "is this posting still open", "what should I apply to next", "evaluate this posting", "tailor my resume for this role", "write the cover letter", "prep me for this interview", "draft a cold email", "is my résumé gate clean". Works with any AI assistant that can read and write files.
---
...
## Which protocol to open

`protocols/` holds four lane protocols plus the shared decision tree they all cite. Each is
self-contained: producer of record, rules with their reason, gate order, how to add the next one.
Open one when the request is about running or gating a specific lane, not about which prompt to
run next (that table is below).

| Request | Protocol | Gate command |
|---|---|---|
| "find/verify a posting" | `protocols/SEARCH.md` | `node tools/ats-sweep.mjs --slugs slugs.json` |
| "build/check my résumé" | `protocols/RESUME.md` | `python tools/verify-resume.py <docx>` |
| "write/check a cover letter" | `protocols/LETTERS.md` | `python tools/verify-letter.py --only <target>` |
| "draft/check a cold email" | `protocols/OUTREACH.md` | `python tools/verify-outreach.py --only <target>` |
| "is this lead worth pursuing" | `protocols/GATES.md` | walk G0–G9, first failure names the verdict |
...
job-search/
  ...
  protocols/                 ← SEARCH.md, RESUME.md, LETTERS.md, OUTREACH.md, GATES.md
  AMMO-LEDGER.md
  CLAIMS-EVIDENCE-LEDGER.md
  resume.md                  ← source of truth
  resume-content.py          ← content module for tools/make-resume.py (optional, v3)
  OPPORTUNITY-TRACKER.md
  VERIFY-LEDGER-<date>.md
  slugs.json
  sweep-<date>.md            ← generated by tools/ats-sweep.mjs
  letters/                   ← <target>.md content files (optional, v3)
    postings/                ← saved posting text, one file per target
    before/                  ← earlier drafts, for the letter gate's negative control
  outreach/                  ← <target>.md cold-message files (optional, v3)
